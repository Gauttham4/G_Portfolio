...
// ============================================================
// Zyntra — Agency Reel (kinetic, color-block scene swaps)
// ============================================================
export function ZyntraAgencyLayout({ theme }: Props) {
  const reduce = useReducedMotion();
  ...
  // Color-block scene swaps
  const bgColor = useTransform(
    scrollYProgress,
    [0, 0.25, 0.5, 0.75, 1],
    [INK, PINK, YELLOW, INK, PINK],
  );
  const textColor = useTransform(
    scrollYProgress,
    [0, 0.25, 0.5, 0.75, 1],
    ['#FFFFFF', '#FFEE00', '#0A0A0A', '#FF0099', '#FFFFFF'],
  );

  // Marquee letters
  const marqueeX = useTransform(scrollYProgress, [0, 1], reduce ? ['0%', '0%'] : ['0%', '-40%']);
  ...
  return (
    ...
    >
      {/* HERO — letter cascade Z·Y·N·T·R·A */}
      <section className="relative min-h-screen px-6 pt-40 pb-20 md:px-12">
        <motion.p
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        ...
        >
          § Digital agency · Reel 01
        </motion.p>

        <div className="mt-16 flex flex-wrap font-display uppercase leading-[0.78] tracking-tight">
          {Array.from('ZYNTRA').map((ch, i) => (
            <motion.span
              key={i}
              initial={{ y: 200, opacity: 0 }}
              animate={{ y: 0, opacity: 1 }}
              transition={{ duration: 1.0, delay: 0.1 + i * 0.08, ease: EASE }}
              style={{ fontSize: 'clamp(4rem, 18vw, 16rem)', display: 'inline-block', fontWeight: 800 }}
            >
              {ch}
            </motion.span>
          ))}
        ...
        <motion.p
          ...
          className="mt-12 max-w-3xl font-display italic"
          style={{ fontSize: 'clamp(1.5rem, 3vw, 2.5rem)', lineHeight: 1.1 }}
        >
          {theme.subtitle}
        </motion.p>

        {/* Scroll-cued marquee strip */}
        <motion.div
          className="pointer-events-none mt-20 whitespace-nowrap font-display uppercase italic"
          style={{ x: marqueeX, fontSize: 'clamp(3rem, 12vw, 10rem)', lineHeight: 1, fontWeight: 800 }}
        >
          NO FRAMEWORK · NO RESTRAINT · NO FRAMEWORK · NO RESTRAINT · NO FRAMEWORK ·
        </motion.div>
      </section>

      {/* SCENE 2 — Story quotes in giant pull style */}
        ...
        <div className="mx-auto max-w-6xl space-y-32">
          {theme.story.quotes.map((q, i) => (
            <motion.div
              ...
              className={i % 2 === 0 ? 'max-w-3xl' : 'max-w-3xl ml-auto text-right'}
            >
              <p className="font-mono text-[10px] uppercase tracking-[0.5em] opacity-60">{q.kicker}</p>
              <p
                className="mt-6 font-display italic"
                style={{ fontSize: 'clamp(2rem, 6vw, 4.5rem)', lineHeight: 1.05, fontWeight: 800 }}
              >
                &ldquo;{q.quote}&rdquo;
              </p>
              <p className="mt-6 max-w-2xl font-sans text-base md:text-lg opacity-80">{q.context}</p>
            </motion.div>
          ))}
        </div>
      </section>

      {/* SCENE 3 — Big bento features */}
      <section className="px-6 py-24 md:px-12 md:py-32">
        <div className="mx-auto max-w-7xl">
          <p className="font-mono text-[10px] uppercase tracking-[0.5em]">§ The toolkit</p>
          <div className="mt-10 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {theme.features.map((f, i) => (
              <motion.article
                ...
                className="border-4 border-current p-6 md:p-8"
                style={{ minHeight: '14rem' }}
              >
                <p className="font-mono text-[10px] uppercase tracking-[0.3em] opacity-70">
                  N° {String(i + 1).padStart(2, '0')}
                </p>
                <h3 className="mt-3 font-display text-2xl md:text-3xl" style={{ fontWeight: 800 }}>
                  {f.title}
                </h3>
                <p className="mt-3 font-sans text-sm md:text-base opacity-85">{f.desc}</p>
              </motion.article>
            ))}
          </div>
        </div>
      ...
      {/* SCENE 4 — Gallery slide-strip */}
      <section className="overflow-hidden py-24 md:py-32">
        <p className="px-6 font-mono text-[10px] uppercase tracking-[0.5em] md:px-12">§ Frames</p>
        <div className="mt-10 flex gap-6 px-6 md:px-12" style={{ overflowX: 'auto' }}>
          {(theme.gallery ?? []).map((g) => (
            <div
              key={g.src}
              className="relative aspect-[3/4] flex-shrink-0 overflow-hidden border-4 border-current"
              style={{ width: 'min(70vw, 380px)' }}
            >
              <Image src={g.src} alt={g.alt} fill sizes="380px" className="object-cover" />
              <div
                className="absolute inset-x-0 bottom-0 px-3 py-2 font-mono text-[9px] uppercase tracking-[0.3em]"
                style={{ background: '#0A0A0AEE', color: YELLOW }}
              >
                {g.caption}
              </div>
            </div>
      ...
      {/* OUTCOME — giant kicker */}
      <section className="px-6 py-32 md:px-12 md:py-48">
        <div className="mx-auto max-w-6xl">
          <p className="font-mono text-[10px] uppercase tracking-[0.5em]">§ Reel out</p>
          <p
            className="mt-10 font-display italic"
            style={{ fontSize: 'clamp(2rem, 7vw, 6rem)', lineHeight: 1.0, fontWeight: 800 }}
          >
            &ldquo;{theme.outcomeQuote}&rdquo;
          </p>
        </div>
      ...
      <NextChapter currentSlug={theme.slug} accent={PINK} />
      <SiteFooter />
    </motion.main>
  );
}
